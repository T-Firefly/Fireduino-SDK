 /**
 * @file boards.h
 * @brief Definitions boards define for Fireduino
...
// Firefly Fireduino
#elif defined(ARDUINO_FIREDUINO)
#define TOTAL_ANALOG_PINS       6
#define TOTAL_PINS              36 // 30 digital + 6 analog
#define VERSION_BLINK_PIN       3
#define VERSION_BLINK_PIN_LOW_IS_ON
//#define PIN_SERIAL1_RX          0
//#define PIN_SERIAL1_TX          1
#define IS_PIN_DIGITAL(p)       ((p) >= 2 && (p) < NUM_DIGITAL_PINS)
#define IS_PIN_ANALOG(p)        ((p) >= NUM_DIGITAL_PINS && (p) < TOTAL_PINS)
#define IS_PIN_PWM(p)           IS_PIN_DIGITAL(p)
#define IS_PIN_SERIAL(p)        ((p) == 0 || (p) == 1)
#define IS_PIN_SERVO(p)         IS_PIN_DIGITAL(p)
#define IS_PIN_I2C(p)           ((p) == 25 || (p) == 26 || (p) == 28 || (p) == 29) // 70 71
#define PIN_TO_DIGITAL(p)       (p)
#define PIN_TO_ANALOG(p)        ((p) - NUM_DIGITAL_PINS)
#define PIN_TO_PWM(p)           PIN_TO_DIGITAL(p)
